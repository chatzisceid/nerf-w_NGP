...
class NeRFSystem(LightningModule):
    def __init__(self, hparams):
        ...
        self.enc = encoding.MultiResHashGrid(3)  # 3D data
        self.dim = 3
        self.batch_size = 100

        # The input value must be within the range [0, 1]
        input = torch.rand((self.batch_size, self.dim), dtype=torch.float32)
        self.enc_input = self.enc(input)

        # Then you can forward into your network
        #self.model = fast_hash(dim=self.enc_input.output_dim, out_dim=1)
        #output = self.model(self.enc_input)

        # Move to other devices
        #self.enc = self.enc.to(dtype='cuda')
...
def main(hparams):
    ...
    ckpt_cb = ModelCheckpoint(dirpath=os.path.join('ckpts', hparams.exp_name),
                        filename='{epoch:d}',
                        monitor='val/psnr',
                        mode='max',
                        save_top_k=-1)
